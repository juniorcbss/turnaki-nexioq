# Configuración ambiente QAS
# Orquestador de módulos para QA/Testing
...
module "dynamodb" {
  ...
  enable_point_in_time_recovery = true # Habilitado en QAS para pruebas
...
module "api_gateway" {
  ...
  throttle_burst_limit = 200 # Mayor capacidad en QAS
  throttle_rate_limit  = 100
  log_retention_days   = 14 # Mayor retención para debugging
...
module "lambda_bookings" {
  ...
  timeout             = 30
  memory_size         = 512
  log_level           = "debug" # Debug en QAS para troubleshooting
  log_retention_days  = 14
  api_gateway_arn     = module.api_gateway.api_execution_arn
...
module "frontend" {
  ...
  project_name      = var.project_name
  environment       = var.environment
  enable_versioning = true # Versionado habilitado en QAS
...
module "waf" {
  ...
  environment  = var.environment
  scope        = "REGIONAL"
  rate_limit   = 3000 # Mayor límite en QAS para load testing
...
module "cloudwatch" {
  ...
  environment            = var.environment
  region                 = var.aws_region
  enable_alarms          = true # Alarmas habilitadas en QAS
