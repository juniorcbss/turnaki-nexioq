<script>
  ...
  async function loadProfessionals() {
    loading = true;
    try {
      const result = await apiClient.get(`/professionals?tenant_id=${TENANT_ID}`);
      professionals = result.professionals || result || [];
    } catch (err) {
      error = err.message;
    } finally {
      loading = false;
  ...
  function selectTab(tab) {
    activeTab = tab;
    if (tab === 'treatments') loadTreatments();
  ...
  async function createTreatment() {
    ...
    success = null;
    try {
      await api.createTreatment({ tenant_id: TENANT_ID, ...newTreatment });
              ...
              <tbody>
                {#each treatments as t}
                  <tr>
                    ...
                    <td>${t.price?.toLocaleString() || 0}</td>
                  </tr>
                {/each}
              </tbody>
            </table>
...
<style>
  ...
  .tabs { display: flex; gap: 0.5rem; border-bottom: 2px solid #e2e8f0; margin-bottom: 2rem; }
  .tab { background: none; border: none; padding: 1rem 2rem; font-weight: 600; color: #64748b; cursor: pointer; border-bottom: 3px solid transparent; transition: all 0.2s; }
  .tab.active { color: #0ea5e9; border-bottom-color: #0ea5e9; }
  .section { background: white; border: 1px solid #e2e8f0; border-radius: 8px; padding: 2rem; margin-bottom: 2rem; }
  h2 { margin: 0 0 1.5rem 0; font-size: 1.25rem; color: #334155; }
  ...
  input { padding: 0.75rem; border: 1px solid #e2e8f0; border-radius: 6px; font-size: 1rem; }
  .btn-primary { background: #0ea5e9; color: white; border: none; padding: 0.75rem 2rem; border-radius: 6px; font-weight: 600; cursor: pointer; align-self: flex-start; }
  .btn-primary:disabled { opacity: 0.6; cursor: not-allowed; }
  .table { overflow-x: auto; }
  table { width: 100%; border-collapse: collapse; }
  th { text-align: left; padding: 0.75rem; background: #f8fafc; border-bottom: 2px solid #e2e8f0; font-weight: 600; color: #334155; }
  td { padding: 1rem 0.75rem; border-bottom: 1px solid #e2e8f0; }
  .alert { margin-bottom: 1rem; padding: 1rem; border-radius: 6px; }
  ...
  .placeholder { text-align: center; padding: 4rem 2rem; color: #94a3b8; font-style: italic; }
  .empty { background: #f8fafc; border: 1px dashed #cbd5e1; border-radius: 8px; padding: 3rem 2rem; }
</style>
